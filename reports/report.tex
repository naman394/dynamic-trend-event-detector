...
\begin{enumerate}
    \item \textbf{Distribution Skew:} The headline length distribution is Gaussian but tightly constrained (mean $\approx$ 42 chars), creating a sparsity challenge for LDA.
    \item \textbf{Non-Stationarity:} We performed a variance-check on tag frequencies, confirming that news narratives are highly non-stationary; certain terms (e.g., "SARS") exist purely as episodic bursts.
    \item \textbf{Tone Distribution:} Real-time GDELT tone distribution shows a significant "Negative Bias" in conflict-themed streams, with an average tone of $-4.2$, indicating that "Impactful" news is often emotionally charged.
\end{enumerate}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{eda/eda_temporal_dist.png}
        \caption{Temporal Distribution of Headlines}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{eda/gdelt_top_themes.png}
        \caption{Top GDELT Themes Frequency}
    \end{subfigure}
...
The core feature of our "Hybrid/Edge" system is the \textbf{Sentence-BERT (SBERT)} embedding. We avoid manual feature engineering by using a pre-trained \texttt{all-MiniLM-L6-v2} model. This model optimizes the objective function:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{baseline/baseline_burst_verification.png}
    \caption{Baseline TF-IDF Temporal Burst Verification}
    \label{fig:baseline}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{advanced_ml/advanced_lda.png}
    \caption{LDA Topic Distribution Visualization}
    \label{fig:advanced_lda}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{deep_learning/deep_learning_clusters.png}
    \caption{SBERT \& HDBSCAN Clusters Projection (BERTopic)}
    \label{fig:bertopic}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{hybrid_impact/semantic_velocity_plot.png}
    \caption{Semantic Velocity Tracking Narrative Shifts Over Time}
    \label{fig:velocity}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{hybrid_impact/impact_distribution.png}
    \caption{Distribution of Event Impact Scores ($S_I$)}
    \label{fig:impact}
\end{figure}
...
\begin{enumerate}
    \item \textbf{LDA Sparsity Failure:} On headlines with $<$5 words, LDA fails because word-co-occurrence vanishes.
    \item \textbf{BERTopic Over-Granularity:} In some runs, BERTopic created 300+ topics.
    \item \textbf{Temporal Lag:} Centroid movement is dependent on bucket size.
\end{enumerate}
